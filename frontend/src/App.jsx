...
export default function App() {
    ...
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-4 bg-slate-100 p-8 text-center">
        ...
        <p className="max-w-md text-sm text-slate-600">
          Make sure is running on <code className="rounded bg-slate-200 px-1"></code>
        </p>
        <button
          onClick={refresh}
  ...
  return (
    <div className="flex min-h-screen bg-board">
      ...
      />
      {selectedGroup ? (
        <BoardView key={selectedGroup.id} groupId={selectedGroup.id} groupName={selectedGroup.name} />
      ) : (
