...
export default function AddTableForm({ onAdd }) {
  ...
  if (!open) {
    return (
      <button
        ...
        className="flex h-fit w-72 shrink-0 items-center justify-center rounded-xl bg-white/20 px-4 py-3 text-sm font-medium text-white transition hover:bg-white/30"
      >
        + Add another table
      </button>
    )
  ...
  return (
    <form
      ...
      className="flex h-fit w-72 shrink-0 flex-col gap-2 rounded-xl bg-column p-3"
    >
      <input
        value={name}
