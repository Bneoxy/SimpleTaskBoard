...
export default function TableColumn({ table, onAddCard, onCardClick, onDeleteTable }) {
    ...
    return (
        <div className="flex h-full w-64 shrink-0 flex-col rounded-xl bg-column/90 sm:w-72">
            <div className="flex items-center justify-between px-3 py-3">
                <h3 className="truncate text-sm font-semibold text-slate-700">{table.name}</h3>
          ...
          <span className="rounded-full bg-slate-300/60 px-2 py-0.5 text-xs text-slate-600">
            ...
            <SortableContext items={cardIds} strategy={verticalListSortingStrategy}>
                <div
                    ref={setNodeRef}
                    className={`flex min-h-0 flex-1 flex-col gap-2 overflow-y-auto px-2 pb-2 transition ${
